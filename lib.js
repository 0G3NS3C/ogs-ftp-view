...
function _organizeFiles(rules, tree, data = {}, path = null) {
  path = path || './';
  for(let entry in tree) {
    if (Number.isInteger(parseInt(entry))) {
      var name = tree[entry].split(".");
      const type = name[1] || null;
            name = name[0] || null;
      if (name && type) {
        var arrayType = "";
        arrayType = _searchInRules(type,rules);
        if (!data[arrayType]) data[arrayType] = [];
        var exist = data[arrayType].find((e) => { return !(e.name !== tree[entry]) })
        if (!exist) {
          data[arrayType].push({
            name: tree[entry],
            path: path+tree[entry]
          })
        }
      }
    }
    else {
      _organizeFiles(rules, tree[entry], data, path+entry+'/')
    }
  }
  return data;
}

function _searchInRules(type,rules) {
  for (let r in rules) {
    if (rules[r].indexOf(type) > -1) return r.toLowerCase();
  }
  return 'unknown';
}
